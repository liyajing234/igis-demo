...
class DrawGraphical {
    ...
    drawShape(positionData) {
        ...
        if (_this.drawingMode === "line") {
        ...
        } else if (_this.drawingMode === "polygon") {
            shape = viewer.entities.add({
                polygon: {
        ...
        } else if (_this.drawingMode === "circle") {
            ...
            //r = r ? r : r + 1;
            shape = viewer.entities.add({
                position: _this.activeShapePoints[0],
                type: "circle",
                ellipse: {
        ...
        } else if (_this.drawingMode === "rectangle") {
            //当positionData为数组时绘制最终图，如果为function则绘制动态图
    ...
    terminateShape() {
        ...
        if (_this.activeShapePoints.length) {
            const shape = this.drawShape(_this.activeShapePoints); //绘制最终图
            _this.polygonList = shape;
        }
        viewer.entities.remove(_this.floatingPoint); //去除动态点图形（当前鼠标点）
        viewer.entities.remove(_this.activeShape); //去除动态图形
        ...
        _this.handler.setInputAction(function (event) {
            ...
            if (Cesium.defined(earthPosition)) {
                if (_this.activeShapePoints.length === 0) {
                    ...
                    _this.activeShape = _this.drawShape(dynamicPositions); //绘制动态图
                }
                _this.activeShapePoints.push(earthPosition);
